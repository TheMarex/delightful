...
function update_data(force_update)
	...
	pacmd_string:gsub('(.-)\n', function(line)
			...
			-- parse volume
			line:gsub('^%s+volume:%s+front%-left:%s+%d+%s+/%s+(%d+)%%.*$', function(match)
					if not sink_num_ok then
						return
					end
...
function load(self, config)
	...
	for sink_id = 1, number_of_sinks do
		...
		if fg_color and fg_center_color and fg_end_color then
            widget:set_color({ type = "linear", from = { 0, 0 }, to = { 0, 19 }, stops = { { 0, fg_color }, { 0.5, fg_center_color }, { 1, fg_end_color} }})
		end
		widget:set_width(8)
